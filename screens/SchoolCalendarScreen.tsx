...
const SchoolCalendarScreen: React.FC = () => {
    ...
    const updateAndPersistEvents = (newEvents: CalendarEvent[]) => {
    ...
    useEffect(() => {
        const loadAndProcessHolidays = async () => {
            ...
            const processedEvents = currentEvents.map(event => {
                ...
                // Revert cancellation if holiday was removed
                if (event.type === CalendarEventType.CLASS && !allHolidayDates.has(event.date) && event.status === ClassStatus.CANCELED) {
                     return { ...event, status: ClassStatus.NORMAL };
                }
                return event;
            });
            ...
            nationalHolidays.forEach(holiday => {
                if (!currentEvents.some(e => e.date === holiday.date && e.title === holiday.title)) {
                    finalEvents.push(holiday);
                }
            });
            
            updateAndPersistEvents(finalEvents);
        };
    ...
    const handleToggleHour = (hour: string) => {
        setExpandedHours(prev => 
            prev.includes(hour) ? prev.filter(h => h !== hour) : [hour]
        );
    };
    ...
    const handleTakeAttendance = (eventToAttend: CalendarEvent) => {
        const courseParts = eventToAttend.course?.split(' - ');
        if (courseParts && courseParts.length === 2) {
            const career = courseParts[0].trim();
            const year = courseParts[1].trim();
            const subject = eventToAttend.title;
            navigate('/tomar-asistencia', { state: { career, year, subject } });
        } else {
    ...
    const handleCancelClass = (eventToCancel: CalendarEvent) => {
        updateAndPersistEvents(events.map(e => e.id === eventToCancel.id ? { ...e, status: ClassStatus.CANCELED } : e));
        setMenuState(null);
    };

    const handleUncancelClass = (eventToRestore: CalendarEvent) => {
        updateAndPersistEvents(events.map(e => e.id === eventToRestore.id ? { ...e, status: ClassStatus.NORMAL } : e));
        setMenuState(null);
    };
    
    const handleDeleteEvent = (eventToDelete: CalendarEvent) => {
        updateAndPersistEvents(events.filter(e => e.id !== eventToDelete.id));
        setMenuState(null);
    };
    ...
    const handleConfirmSwap = (event1: CalendarEvent, event2: CalendarEvent) => {
        ...
        if (index1 !== -1 && index2 !== -1) {
             const original1 = { date: newEvents[index1].date, startTime: newEvents[index1].startTime, endTime: newEvents[index1].endTime };
            const original2 = { date: newEvents[index2].date, startTime: newEvents[index2].startTime, endTime: newEvents[index2].endTime };
            newEvents[index1] = { ...newEvents[index1], date: original2.date, startTime: original2.startTime, endTime: original2.endTime, isSwapped: true, originalDate: original1.date, originalStartTime: original1.startTime, originalEndTime: original1.endTime };
            newEvents[index2] = { ...newEvents[index2], date: original1.date, startTime: original1.startTime, endTime: original1.endTime, isSwapped: true, originalDate: original2.date, originalStartTime: original2.startTime, originalEndTime: original2.endTime };
        }
        updateAndPersistEvents(newEvents);
    ...
    const handleRevertSwap = (eventToRevert: CalendarEvent) => {
        ...
        const otherSwappedEvent = events.find(e => e.isSwapped && e.date === eventToRevert.originalDate && e.startTime === eventToRevert.originalStartTime && e.originalDate === eventToRevert.date && e.originalStartTime === eventToRevert.startTime);

        if (!otherSwappedEvent) {
            alert('No se pudo encontrar la clase correspondiente para revertir.');
            setMenuState(null);
            return;
        }
        ...
        if (index1 !== -1 && index2 !== -1) {
            newEvents[index1] = { ...newEvents[index1], date: eventToRevert.originalDate!, startTime: eventToRevert.originalStartTime, endTime: eventToRevert.originalEndTime, isSwapped: undefined, originalDate: undefined, originalStartTime: undefined, originalEndTime: undefined };
            newEvents[index2] = { ...newEvents[index2], date: otherSwappedEvent.originalDate!, startTime: otherSwappedEvent.originalStartTime, endTime: otherSwappedEvent.originalEndTime, isSwapped: undefined, originalDate: undefined, originalStartTime: undefined, originalEndTime: undefined };
        }
        updateAndPersistEvents(newEvents);
        setMenuState(null);
    };
    ...
    const TimetableItem: React.FC<{ event: CalendarEvent }> = ({ event }) => {
        ...
        const originalDateFormatted = event.originalDate ? new Date(event.originalDate + 'T00:00:00').toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit' }) : '';
        ...
        return (
            <div className={`w-full text-left ${cardBg} p-3 rounded-lg mb-2 flex items-start justify-between space-x-2 transition-all ${isCanceled ? 'opacity-70' : ''}`}>
                <div className="flex-grow min-w-0">
                    ...
                    {isSwapped && (<p className="text-xs text-amber-700 mt-1">Originalmente: {originalDateFormatted} {event.originalStartTime}</p>)}
                </div>
                <div className="flex items-center space-x-2 flex-shrink-0">
                    {isCanceled ? <span className="text-xs h-fit font-semibold text-slate-700 bg-slate-200 px-2 py-1 rounded-full">Cancelada</span>
                     : event.attendanceTaken ? <span className="text-xs h-fit font-semibold text-green-700 bg-green-100 px-2 py-1 rounded-full">Registrada</span>
                     : <span className="text-xs h-fit font-semibold text-slate-600 bg-slate-200 px-2 py-1 rounded-full">Pendiente</span>}
                    <div className="relative"><button onClick={(e) => handleOpenMenu(event, e)} className="p-1.5 rounded-full hover:bg-slate-200"><EllipsisVerticalIcon className="w-5 h-5 text-slate-600"/></button></div>
                </div>
            </div>
        );
    };
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={onClose}>
                <div className="bg-white rounded-lg shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                    <form onSubmit={handleSubmit}>
                        <div className="p-6 border-b"><h3 className="text-lg font-bold">Agregar Evento</h3><p className="text-sm text-slate-500">para el {selectedDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'long' })}</p></div>
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                <select id="eventType" value={type} onChange={e => setType(e.target.value as CalendarEventType)} className={inputStyle}>
                                    {Object.entries(addableEventTypes).map(([key, label]) => (<option key={key} value={key}>{label}</option>))}
                                </select>
                            </div>
                             <div>
    ...
    const ConfigModal: React.FC<{ isOpen: boolean; onClose: () => void; }> = ({ isOpen, onClose }) => {
        ...
        const handleSave = () => {
          if (startDate && endDate) { setSchoolYear(startDate, endDate, winterStart, winterEnd); onClose(); alert('Ciclo lectivo configurado.'); } else { alert('Por favor, selecciona las fechas de inicio y fin del ciclo.'); }
        };
      
        return (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b flex items-center space-x-3"><CalendarDaysIcon className="w-6 h-6 text-indigo-700"/><div><h3 className="text-lg font-bold">Configurar Ciclo Lectivo</h3><p className="text-sm text-slate-500">Define las fechas importantes del año.</p></div></div>
                <div className="p-6 space-y-4">
                    <div>
                        <label htmlFor="start-date" className="block text-sm font-medium text-slate-700 mb-1">Inicio del Ciclo</label>
                         <div className="relative">
                            ...
                            <button type="button" onClick={() => handleDateIconClick(startDateRef)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-600" aria-label="Seleccionar fecha de inicio del ciclo"><CalendarDaysIcon className="w-5 h-5" /></button>
                         </div>
                    </div>
                    <div>
                        ...
                        <div className="relative">
                            <input ref={endDateRef} type="date" id="end-date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full p-2 pr-10 border border-slate-300 rounded-md bg-white"/>
                             <button type="button" onClick={() => handleDateIconClick(endDateRef)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-600" aria-label="Seleccionar fecha de fin del ciclo"><CalendarDaysIcon className="w-5 h-5" /></button>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <button type="button" onClick={() => handleDateIconClick(winterStartRef)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-600" aria-label="Seleccionar fecha de inicio de receso"><CalendarDaysIcon className="w-5 h-5" /></button>
                        </div>
                    </div>
                     <div>
                        ...
                        <div className="relative">
                            <input ref={winterEndRef} type="date" id="winter-end-date" value={winterEnd} onChange={e => setWinterEnd(e.target.value)} className="w-full p-2 pr-10 border border-slate-300 rounded-md bg-white"/>
                             <button type="button" onClick={() => handleDateIconClick(winterEndRef)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-600" aria-label="Seleccionar fecha de fin de receso"><CalendarDaysIcon className="w-5 h-5" /></button>
                        </div>
                    </div>
                </div>
                <div className="p-4 bg-slate-50 flex justify-end space-x-2 rounded-b-lg">
    ...
    const ActionMenuItems: React.FC<{ event: CalendarEvent; itemClassName: string }> = ({ event, itemClassName }) => {
        ...
        const handleAction = (action: (event: CalendarEvent) => void) => action(event);
    ...
    const ActionMenu: React.FC = () => {
        ...
        return (
            <div ref={menuRef} style={{ top: menuState.position?.top, left: menuState.position?.left }} className="fixed bg-white rounded-lg shadow-xl border border-slate-200 z-50 w-56">
                <ActionMenuItems event={menuState.event} itemClassName="w-full text-left flex items-center space-x-3 px-4 py-2" />
            </div>
        );
    };
    ...
    const SwapClassModal: React.FC<{
        ...
        useEffect(() => {
            ...
            } else { setTargetDate(null); setTargetEvent(null); setDisplayWeek([]); }
        }, [isOpen, eventToSwap]);

        if (!isOpen || !eventToSwap) return null;
        
        const handlePrevWeek = () => { const firstDay = displayWeek[0]; const prevWeekStart = new Date(firstDay.setDate(firstDay.getDate() - 7)); setDisplayWeek(getWeekDays(prevWeekStart)); setTargetDate(null); setTargetEvent(null); };
        const handleNextWeek = () => { const firstDay = displayWeek[0]; const nextWeekStart = new Date(firstDay.setDate(firstDay.getDate() + 7)); setDisplayWeek(getWeekDays(nextWeekStart)); setTargetDate(null); setTargetEvent(null); };

        const potentialSwapTargets = allEvents.filter(e => e.type === CalendarEventType.CLASS && e.status !== ClassStatus.CANCELED && e.date === targetDate?.toISOString().split('T')[0] && e.id !== eventToSwap.id && e.course === eventToSwap.course);
                ...
                <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto">
                        <div className="space-y-4">
                            ...
                            <div className="bg-slate-100 p-3 rounded-lg border border-slate-200"><p className="font-bold">{eventToSwap.title}</p><p className="text-sm">{eventToSwap.course}</p><p className="text-sm text-slate-500">{new Date(eventToSwap.date + 'T00:00:00').toLocaleDateString('es-ES', { weekday: 'long' })} - {eventToSwap.startTime}</p></div>
                            ...
                            <div className="bg-slate-50 p-2 rounded-lg">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <span className="font-semibold text-sm">{displayWeek.length > 0 ? `${monthNames[displayWeek[0].getMonth()]} ${displayWeek[0].getFullYear()}` : ''}</span>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                                {potentialSwapTargets.length > 0 ? potentialSwapTargets.map(target => (<button key={target.id} onClick={() => setTargetEvent(target)} className={`w-full text-left p-3 rounded-lg border-2 ${targetEvent?.id === target.id ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 hover:bg-slate-50'}`}><p className="font-semibold">{target.startTime} - {target.title}</p><p className="text-sm text-slate-500">{target.course}</p></button>)) : <p className="text-sm text-slate-500 text-center py-4">No hay clases de la misma carrera/año para intercambiar en este día.</p>}
                            </div>
                        </div>
                    </div>
                    <div className="p-4 bg-slate-50 flex justify-end space-x-2 rounded-b-lg">
            ...
            <div className="hidden lg:grid lg:grid-cols-3 lg:gap-6">
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white p-4 rounded-lg shadow-sm">
                        ...
                        <div className="grid grid-cols-7 gap-1 place-items-center">{viewMode === 'month' ? (<>{Array.from({ length: firstDayOfMonth }).map((_, i) => <div key={`empty-${i}`}></div>)}{monthDays.map(day => <CalendarDay key={day.toISOString()} day={day} isMini />)}</>) : (weekDays.map(day => <CalendarDay key={day.toISOString()} day={day} isMini />))}</div>
                    </div>
                    <ColorLegend />
                     <div className="bg-white p-4 rounded-lg shadow-sm">
                ...
                <div className="lg:col-span-2">
                     <div className="bg-white p-6 rounded-lg shadow-sm h-full">
                        ...
                        <div className="mt-6 space-y-4 max-h-[calc(100vh-220px)] overflow-y-auto pr-3">{sortedAgendaHours.length > 0 ? sortedAgendaHours.map(hour => (<div key={hour}><button onClick={() => handleToggleHour(hour)} className="w-full flex justify-between items-center text-left p-3 bg-slate-50 rounded-lg"><h4 className="font-bold text-indigo-700">{hour} hs</h4>{expandedHours.includes(hour) ? <ChevronUpIcon className="w-5 h-5 text-indigo-700"/> : <ChevronDownIcon className="w-5 h-5 text-indigo-700"/>}</button>{expandedHours.includes(hour) && (<div className="pt-3">{agendaByHour[hour].map(event => <TimetableItem key={event.id} event={event} />)}</div>)}</div>)) : (<div className="flex items-center justify-center h-80 text-center text-slate-500"><p>No hay clases programadas para este día.</p></div>)}</div>
                    </div>
                </div>
            </div>
